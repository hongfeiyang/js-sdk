...
import { ClientTask, Item, ShareWithItemData, Slot } from '@meeco/vault-api-sdk';
...
function stubVault(api: nock.Scope) {
  ...
  api
    .put(`/client_task_queue`, (body: any) => body.client_tasks.every(x => x.state === ClientTaskState.Done))
    .matchHeader('Authorization', testUserAuth.vault_access_token)
    .matchHeader('Meeco-Subscription-Key', 'environment_subscription_key')
